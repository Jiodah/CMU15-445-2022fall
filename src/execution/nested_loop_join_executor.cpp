...
auto NestedLoopJoinExecutor::InnerJoin(const Value &cmp, const Schema &schema, Tuple *tuple) -> bool {
  ...
  if (index_ != 0) {
    for (uint32_t j = index_; j < right_tuples_.size(); j++) {
      ...
      if (plan_->Predicate().EvaluateJoin(&left_tuple_, left_schema_, &right_tuples_[j], right_schema_).CompareEquals(cmp) == CmpBool::CmpTrue) {
  ...
  if (index_ == 0) {
    while (left_executor_->Next(&left_tuple_, &left_rid_)) {
      for (const auto &right_tuple : right_tuples_) {
        index_ = (index_ + 1) % right_tuples_.size();
        if (plan_->Predicate().EvaluateJoin(&left_tuple_, left_schema_, &right_tuple, right_schema_).CompareEquals(cmp) == CmpBool::CmpTrue) {
          std::vector<Value> value;
          for (uint32_t i = 0; i < left_schema_.GetColumnCount(); i++) {
            value.push_back(left_tuple_.GetValue(&left_schema_, i));
          }
